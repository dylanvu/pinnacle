...
roomCode = None # code for the host room code
hostID = None # code for the host ID on backend purposes

sio = socketio.Client()
@sio.event
def connect():
    print('connection established')
    sio.emit("host connection", {})
...
def disconnect():
    ...
    global roomCode
    global hostID
    sio.emit('host disconnection', {'hostID' : hostID, 'roomID' : roomCode})

@sio.on('room and host code')
def saveCodes(data):
    print("boop")
    print(data)
    global roomCode
...
while running:
    ...
    if pygame.mouse.get_pressed()[0]:
        ...
        sio.emit('drawn', {'hostID' : hostID, 'prevCoord' : prevMouse, 'currCoord' : mouse})
